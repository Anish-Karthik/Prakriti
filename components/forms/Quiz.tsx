...
import { TquestionMCQ, questionMCQarray } from "@/lib/questions"
...
const Quiz = () => {
  ...
  const [answerMCQarray, setAnswers] = useState<TAnswer[]>(
    new Array(questionMCQarray.length).fill({
      answer: "",
      type: "initial",
      qno: -1,
      option: -1,
  ...
  async function submitAnswers() {
    ...
    window.localStorage.setItem(
      "prakriti",
      JSON.stringify(response.data.prakriti)
    )
  ...
  return (
    <div
      className="rounded-md flex flex-col items-center justify-center xl:w-2/3 gap-4 bg-slate-100 px-4 py-2 lg:-mt-16 sm:-mt-8"
      style={
        {
          // media query
        }
    ...
    >
      <div className="w-full  rounded-md">
        ...
        <h1 className="text-2xl text-center">
          Q.{Math.min(index + 1, questionMCQarray.length)} of{" "}
          {questionMCQarray.length}
        </h1>
      </div>
